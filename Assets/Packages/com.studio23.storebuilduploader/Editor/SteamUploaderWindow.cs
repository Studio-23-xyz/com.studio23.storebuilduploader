...
namespace Studio23.StoreBuildUploader.Editor
{
    public class SteamUploaderWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            if (config != null)
            {
                ...
                {
                    string selectedPath =
                        EditorUtility.OpenFolderPanel("Select Steamworks SDK Path", config.steamSDKPath, "");
                ...
                {
                    string selectedPath =
                        EditorUtility.OpenFolderPanel("Select Build Folder Path", config.buildFolderPath, "");
                    if (!string.IsNullOrEmpty(selectedPath))
                    {
        ...
        private bool ValidateSDKPath(string sdkPath)
        {
            ...
            string vdfPath = Path.Combine(sdkPath, "tools/ContentBuilder/scripts/",
                string.Format(vdfFileNameTemplate, config.appId));
        ...
        private void GenerateVdfFile(SteamUploadConfig config)
        {
            ...
            string vdfPath = Path.Combine(config.steamSDKPath, "tools/ContentBuilder/scripts/",
                string.Format(vdfFileNameTemplate, config.appId));
        ...
        private string GenerateVdfContent(SteamUploadConfig config)
        {
            ...
            return $"\"AppBuild\"\n" +
                   ...
                   $"\t\t\t\"FileExclusion\"\n" +
                   $"\t\t\t{{\n" +
                   $"\t\t\t\t\"LocalPath\" \"BurstDebugInformation_DoNotShip/*\"\n" +
                   $"\t\t\t}}\n" +  // Exclude the BurstDebugInformation_DoNotShip folder
                   $"\t\t}}\n" +
                   $"\t}}\n" +
                   $"}}";
        }
        ...
        private void OpenSteamworksContentFolder(SteamUploadConfig config)
        {
            ...
            if (Directory.Exists(contentFolderPath))
            {
                ...
                Process.Start("explorer.exe",
                    "/select," + contentFolderPath); // Opens file explorer with the specified path selected
            }
            else
            {
        ...
        private void UpdateRunBuildBat(SteamUploadConfig config)
        {
            ...
            string vdfFilePath = Path.Combine(config.steamSDKPath, "tools/ContentBuilder/scripts/",
                $"app_build_{config.appId}.vdf");
